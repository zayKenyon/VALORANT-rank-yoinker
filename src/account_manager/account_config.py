...
class AccountConfig:
    def __init__(self, log):
        ...
        self.pritvate_settings = os.path.join(
    ...
    def get_riot_client_path(self):
        path = os.path.join(
            os.getenv("ALLUSERSPROFILE"), R"Riot Games\RiotClientInstalls.json"
        )
    ...
    def load_accounts_config(self):
        ...
        try:
            with open(
                os.path.join(os.getenv("APPDATA"), "vry/accounts.json"), "r"
            ) as f:
            ...
            try:
                if len(yaml_data["riot-login"]["persist"]["session"]["cookies"]) == 5:
                    for i, cookie in enumerate(
                        yaml_data["riot-login"]["persist"]["session"]["cookies"]
                    ):
                        yaml_data["riot-login"]["persist"]["session"]["cookies"][i][
                            "value"
                        ] = account_data["cookies"].get(cookie["name"])
            except TypeError:
