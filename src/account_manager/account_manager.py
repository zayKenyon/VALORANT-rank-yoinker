...
class AccountManager:
    ...
    def menu_change_accounts(self):
        ...
        if result["menu"] == "Add new account":
            ...
            if option == 0:
                questions = [
                    {
                        "type": "input",
                        "message": "Please type username of the account you want to add:",
                        "name": "username",
                    },
                    {
                        "type": "password",
                        "message": "Please type password of the account you want to add:",
                        "name": "password",
                    },
                ]
                try_again = True
                while try_again:
                    ...
                    if current_account_auth_data is None:
                        try_again = inquirer.confirm(
                            message="Invalid username or password! Do you want to try again?",
                            default=True,
                        ).execute()
                    else:
                        try_again = False
        ...
        # Remove account
            ...
            if (
                self.last_account_data["name"]
                == self.account_config.accounts_data[account]["name"]
            ):
