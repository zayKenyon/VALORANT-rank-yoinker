...
class AccountAuth:
    ...
    def auth_account(self, username=None, password=None, cookies=None):
        ...
        r_entitlements = self.session.post(
            "https://entitlements.auth.riotgames.com/api/token/v1",
            headers={"Authorization": "Bearer " + access_token} | self.headers,
            json={},
        )
    ...
    def get_latest_season_id(self):
        ...
        if self.content is None:
            self.content = requests.get(
                f"https://shared.{self.region}.a.pvp.net/content-service/v3/content",
    ...
    def get_account_data(self):
        ...
        r_mmr = requests.get(
            f"https://pd.{self.region}.a.pvp.net/mmr/v1/players/{self.puuid}",
            headers=self.auth_headers,
            verify=False,
        )
        if (
            r_mmr.json()["QueueSkills"]["competitive"].get("SeasonalInfoBySeasonID")
            is not None
        ):
            season_info = r_mmr.json()["QueueSkills"]["competitive"][
                "SeasonalInfoBySeasonID"
            ].get(self.get_latest_season_id())
        ...
        name = requests.put(
            f"https://pd.{self.region}.a.pvp.net/name-service/v2/players",
            headers=self.auth_headers,
            json=[self.puuid],
        ).json()
        name = name[0]["GameName"] + "#" + name[0]["TagLine"]
        r_account_xp = requests.get(
            f"https://pd.{self.region}.a.pvp.net/account-xp/v1/players/{self.puuid}",
            headers=self.auth_headers,
            verify=False,
        )
        ...
        contracts = [
            a
            for a in contracts.json()["data"]
            if a["content"]["relationType"] == "Season"
        ]
        bp = contracts[-1]
        r_contracts = requests.get(
            f"https://pd.{self.region}.a.pvp.net/contracts/v1/contracts/{self.puuid}",
            headers=self.auth_headers,
            verify=False,
        )
        ...
        return {"rank": rank, "name": name, "level": level, "bp_level": bp_level}
    ...
    def ask_for_mfa(self):
        ...
        return prompt(
            {
                "type": "input",
                "message": "Please enter your MFA/2FA code:",
                "name": "mfa",
            }
        )["mfa"]
