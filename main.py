...
language = input("Which language is your preference? / Hangi dili tercih ediyorsun? (TR/EN): ")
if language == ("EN"):
...
if language == ("TR"):
    ...
    os.system('cls')
    os.system(f"title Valorant Oyuncu Bilgi Gösterici v{version}")

    server = ""


    def program_exit(status: int):  # so we don't need to import the entire sys module
        log(f"Program şu hata kodu ile çöktü: {status}")
        raise sys.exit(status)
    ...
    try:
        ...
        log(f"Valorant Oyuncu Bilgi Gösterici v{version}")
        ...
        while True:
            ...
            try:
                ...
                raise Exception("Oyun daha başlamadı!")
            if cfg.cooldown == 0 or game_state != lastGameState:
                log(f"getting new {game_state} scoreboard")
                lastGameState = game_state
                game_state_dict = {
                    "INGAME": color('Oyunda', fore=(241, 39, 39)),
                    "PREGAME": color('Ajan Seçiminde', fore=(103, 237, 76)),
                    "MENUS": color('Menüde', fore=(238, 241, 54)),
                }
                if game_state == "INGAME":
                    coregame_stats = coregame.get_coregame_stats()
                    Players = coregame_stats["Players"]
                    ...
                    names = namesClass.get_names_from_puuids(Players)
                    loadouts = loadoutsClass.get_match_loadouts(coregame.get_coregame_match_id(), Players, cfg.weapon, valoApiSkins, names, state="game")
                    with alive_bar(total=len(Players), title='Oyuncuların verileri çekiliyor', bar='classic2') as bar:
                        presence = presences.get_presence()
                        partyOBJ = menu.get_party_json(namesClass.get_players_puuid(Players), presence)
                        log(f"retrieved names dict: {names}")
                        Players.sort(key=lambda Players: Players["PlayerIdentity"].get("AccountLevel"), reverse=True)
                        ...
                        for player in Players:
                            ...
                            rankStatus = playerRank[1]
                            while not rankStatus:
                                print("Çok fazla programı aç kapa yapıyorsun kısa süreliğine banlandın, 😞 10 saniye bekle!")
                                time.sleep(10)
                                playerRank = rank.get_rank(player["Subject"], seasonID)
                                rankStatus = playerRank[1]
                            playerRank = playerRank[0]
                ...
                if (title := game_state_dict.get(game_state)) is None:
                    ...
                    time.sleep(9)
                if server != "":
                    table.title = f"Şu anki Valorant Durumu: {title} - {server}"
                else:
                    table.title = f"Şu anki Valorant Durumu: {title}"
                server = ""
                table.field_names = ["Parti", "Ajan", "İsim", "Skin", "Rank", "Rank Sıra", "En İyi Rank", "pos.", "Seviye"]
                if title is not None:
                    print(table)
                    print(f"VALORANT Oyuncu Bilgi Gösterici v{version}")
            if cfg.cooldown == 0:
                input("Verileri tekrar çekmek için lütfen Enter'e basınız...")
            else:
                time.sleep(cfg.cooldown)
    except:
        print(color(
            "Program bir hata ile karşılaştı. Eğer problem devam ederse şu dosya klasöründeki loglarla beraber bize ulaşın."
            f" {os.getcwd()}\logs", fore=(255, 0, 0)))
        traceback.print_exc()
